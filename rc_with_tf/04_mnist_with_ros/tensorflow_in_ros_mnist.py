...
class RosTensorFlow():
    ...
    def __init__(self):
        ...
        self._sub = rospy.Subscriber(
            'image', Image, self.callback, queue_size=1)
    ...
    def callback(self, image_msg):
        ...
        # ret, cv_image_binary = cv2.threshold(
        #     cv_image_gray, 128, 255, cv2.THRESH_BINARY_INV)

        # # Adaptive Gaussian Thresholding
        # cv_image_binary = cv2.adaptiveThreshold(cv_image_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        ...
        cv_image_binary = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                    cv2.THRESH_BINARY, 11, 2)

        # # フィルター+大津
        # blur = cv2.bilateralFilter(cv_image_gray, 6, 12, 3)
        # ret, th2 = cv2.threshold(blur, 0, 255,
        ...
        self._imagegray_pub.publish(
            self._cv_bridge.cv2_to_imgmsg(cv_image_binary, 'mono8'))
        self._image28_pub.publish(
            self._cv_bridge.cv2_to_imgmsg(cv_image_28, 'mono8'))

        predict_num = self._session.run(
            self.y_conv, feed_dict={self.x: np_image, self.keep_prob: 1.0})
